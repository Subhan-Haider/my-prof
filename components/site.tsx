  ...
  return (
    <>
      ...
      <header className="fixed z-40 top-3 sm:top-4 left-1/2 -translate-x-1/2 w-[calc(100%-1.25rem)] sm:w-[calc(100%-2rem)] max-w-6xl transition-all duration-300">
        <div
          className={`flex h-14 sm:h-16 items-center justify-between rounded-full px-4 sm:px-6 md:px-7 transition-all duration-300 ${scrolled
              ? "bg-[#090a12]/90 backdrop-blur-xl border border-white/10 shadow-[0_15px_35px_-10px_rgba(0,0,0,0.8)]"
              : "bg-[#0f111d]/75 backdrop-blur-md border border-white/5"
            }`}
        >
          {/* Logo */}
          <Link
            href="/"
            className="flex items-center gap-2 sm:gap-2.5 font-display text-base sm:text-lg font-bold tracking-tight text-white group"
          >
          ...
          <button
            ...
            className="flex md:hidden w-10 h-10 rounded-full items-center justify-center bg-white/5 border border-white/10 text-white min-w-[40px] min-h-[40px] active:scale-95 transition-transform"
          >
          ...
          {open && (
            <>
              ...
              <motion.div
                ...
                className="mt-2 rounded-3xl border border-white/10 bg-[#0f111d]/98 p-5 backdrop-blur-2xl shadow-2xl md:hidden max-h-[85vh] overflow-y-auto"
              >
                <div className="flex items-center gap-2 pb-3 mb-2 border-b border-white/10 text-xs font-mono text-[#34d399]">
                ...
                <div className="space-y-1">
                  {navLinks.map((link) => (
                    <Link
                      ...
                      href={link.href}
                      onClick={() => setOpen(false)}
                      className="flex items-center justify-between py-3 px-3.5 rounded-xl text-sm text-[#cbd5e1] hover:text-white hover:bg-white/5 active:bg-white/10 transition-colors min-h-[44px]"
                    >
                      <span className="font-medium">{link.name}</span>
                      <ArrowUpRight size={15} className="text-[#64748b]" />
                    </Link>
                  ))}
                </div>
                <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-2">
                  <Link
                    href="/contact"
                    onClick={() => setOpen(false)}
                    className="w-full flex items-center justify-center gap-2 py-3.5 rounded-full bg-[#34d399] font-bold text-xs text-[#090a12] shadow-lg active:scale-98 transition-transform min-h-[44px]"
                  >
...
export function Footer() {
  ...
  return (
    <footer className="relative border-t border-white/[0.08] bg-[#07080e] px-5 sm:px-6 pt-20 pb-12 md:px-12 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr] items-start pb-16 border-b border-white/[0.08]">
          <div>
            <div className="font-display text-4xl sm:text-7xl lg:text-8xl font-black tracking-tighter text-white mt-2 break-words">
              SUBHAN<span className="text-[#34d399]">.</span>
            </div>
            <p className="mt-5 max-w-md text-sm sm:text-base leading-relaxed text-[#94a3b8]">
